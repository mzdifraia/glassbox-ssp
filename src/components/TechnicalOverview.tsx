...
export function TechnicalOverview({ status, lastRunMs }: TechnicalOverviewProps) {
  return (
    <section className="rounded-xl border border-zinc-700/80 bg-zinc-900/90 p-4 font-mono text-[11px]">
      <h2 className="font-sans text-sm font-semibold uppercase tracking-wide text-zinc-400">
        System
      </h2>

      <div className="mt-3 grid gap-4 lg:grid-cols-3">
        <div>
          <h3 className="text-[10px] uppercase text-zinc-500">Pipeline (10 gates)</h3>
          <ol className="mt-2 max-h-40 space-y-0.5 overflow-y-auto text-zinc-400">
          ...
          <ul className="mt-2 space-y-1.5 text-zinc-400">
            {API_ROUTES.map((r) => (
              <li key={r.path}>
                ...
                <div className="font-sans text-[10px] text-zinc-600">{r.note}</div>
              </li>
            ))}
          </ul>
          {lastRunMs != null && (
            <p className="mt-2 text-zinc-500">
              Last run: <span className="text-zinc-300">{lastRunMs}ms</span> wall-clock
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <h3 className="mt-3 text-[10px] uppercase text-zinc-500">Code layout</h3>
